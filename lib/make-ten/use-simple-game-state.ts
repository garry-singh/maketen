...
export function useSimpleGameState() {
  ...
  const solvePuzzle = (timeElapsed: number, solutionInput: string): string => {
    ...
    try {
      ...
      localStorage.setItem(STORAGE_KEYS.LONGEST_STREAK, String(newLongestStreak));
      localStorage.setItem(STORAGE_KEYS.SOLVED_TODAY, "true");
      localStorage.setItem(STORAGE_KEYS.SOLVED_DATE, today);
      localStorage.setItem(STORAGE_KEYS.SOLUTION_INPUT, solutionInput);
      localStorage.setItem(STORAGE_KEYS.SOLUTION_TIME, String(timeElapsed));
    } catch (error) {
